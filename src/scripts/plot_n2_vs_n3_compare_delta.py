#!/usr/bin/env python
"""N2 vs N3 infraslow spectrum + slow-wave-phase comparison across a cohort (delta band).

Converted from ``demo_plot_and_compared_delta.ipynb`` -- the delta-band/slow-wave analog of
``plot_n2_vs_n3_compare.py`` (sigma-band/spindle). Downstream analysis/visualization over
`infraslow.pipeline`'s already-computed per-subject ISFS spectrum and phase features (see
``src/scripts/preprocessing.py`` for how the underlying `.npz`/`.csv` artifacts under
``<data-dir>`` were generated). No EEG filtering, spindle/slow-wave detection, or envelope
computation happens here -- every number below comes from
...
that has a finite, usable ISFS spectrum AND non-degenerate, finite slow-wave-phase data
in both states (see ``_is_usable``) -- not just a fixed demo-sized subset. Pass
``--n-subjects`` to cap the count (first N by subject id) or leave it unset to use every
eligible subject found.
...
(the package is not pip-installed), e.g.:
    ...
    srun -p normal --time=00:30:00 --mem=8G --cpus-per-task=4 \\
        python3 plot_n2_vs_n3_compare_delta.py --channel C3 --workers 4
...
def parse_args():
    ...
    p.add_argument('--output-dir', type=Path,
                    default=Path(os.path.expandvars(
                        os.environ.get('OUTPUT_DIR', '/scratch/users/chaisaen/outputs/n2_vs_n3_compare_delta'))),
...
def _is_usable(entry, states=STATES):
    ...
    despite >= 5 total detected slow waves -- `phase_bin_rates` is `[0.0]*8`, NOT `None`, in
...
def plot_spectrum_figure(freqs, band_m, rel_mean, corrected_mean, group_fit, *, channel, n_subjects):
    ...
    fig.suptitle(f"N2 vs N3 average ISFS spectrum, delta band ({channel}, n={n_subjects} subjects)",
                 fontweight="bold")
    fig.tight_layout()
    return fig
...
def plot_phase_distribution_figure(bin_centers, phase_mean, *, channel, n_subjects):
    ...
    ax.set(xlabel="ISFS phase (rad)", ylabel="% of slow waves (of total detected)",
           ...
           title=f"N2 vs N3 slow-wave distribution across ISFS phase ({channel}, n={n_subjects} subjects)")
...
def plot_peak_freq_paired_figure(peak_freqs, *, n_subjects):
    ...
    ax.set(xlim=(0.7, 2.3), xticks=[1, 2], xticklabels=STATES, ylabel="Peak frequency (Hz)",
           title=f"Paired N2 vs N3 peak frequency, delta band (n={n_subjects} subjects)")
...
def main():
    ...
    summary_lines = [
        "Summary",
        "=======",
        f"Subjects: {n_subjects} (channel {args.channel}), paired N2/N3 usable slow-wave/delta data",
