...
2. Per-``CHANNELS`` EEG channel and per-``NPZ_STAGES`` stage group, the
   ...
   spindle/bout detail -- see :func:`calculate_channel_events`, written to disk
   as one ``.npz`` per subject per stage by the caller.
...
def calculate_stage_events(
    ...
    *,
    ...
    4. Per kept bout, slice the channel's dB sigma-power envelope
       ...
       eeg_envelope`) to the bout window and take its infraslow spectrum
    ...
    Args:
        ...
        t_env, sigma_db: This channel's dB sigma-power envelope and its
            ...
            :func:`~infraslow.processing.infraslow.eeg_envelope`) and reused
            across every stage.
    ...
    for a, b in bouts:
        ...
        if seg.size < min_len:
            continue
        try:
            spec = infraslow_spectrum(seg, sf_env, window_sec=window_sec)
        except Exception:  # noqa: BLE001 - one bout's spectrum failing must not drop the others
            continue
        if freqs is None:
            freqs = spec.freqs
        rel = relative_power(spec.psd, freqs, infraslow_band)
        corrected = baseline_correct(rel, freqs, baseline_band)
        if np.all(np.isfinite(rel)) and np.all(np.isfinite(corrected)):
...
def calculate_channel_events(
    ...
    *,
    ...
    (:func:`~infraslow.processing.infraslow.eeg_envelope`, matching
    ...
    for ch in channels:
        ...
        try:
            ...
            t_env, sigma_db = eeg_envelope(
                data, float(loader.sf), band=SIGMA_BAND, sf_env=SF_ENV, smooth_sec=1.0,
                to_db=True, wavelet=True, kind="power",
            )
        except Exception as exc:  # noqa: BLE001 - a missing/failed channel must not sink the subject
