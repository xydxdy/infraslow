...
# Group-selection outputs. Subjects are grouped by apnea severity (AHI), which
# produces the final saved result.
...
def process(df: pd.DataFrame) -> dict:
    """Step 2 — post-merge processing and group selection by apnea severity.
    ...
    Steps:
        ...
        3. Drop subjects whose raw recordings (EDF + Hypnodensity.csv) are not
           present on disk, so only processable subjects remain.
        4. Save the processed DataFrame once, at the last.

    Returns a backward-compatible dict holding the processed DataFrame, the
    final AHI groups restricted to subjects with files on disk
    (``selected_groups``), and the unrestricted final groups
    (``selected_groups_all``).
    """
    # Compute AHI = (Numbers_OA_Total + Numbers_HI_Total) / (TST hours).
    df = compute_ahi(df)

    # Step 2 — two-threshold grouping by apnea severity (AHI), on the in-memory
    # df. This is the final result.
    groups_final: Dict[str, List[str]] = selected_groups_by_ahi(
        df,
        id_column=ID_COLUMN,
    ...
    # Step 3 — keep only subjects whose raw recordings actually exist on disk.
